package com.anhdt.doranewsvermain.constant;

public class RootAPIUrlConst {
    public static final String FOLLOW = "1";
    public static final String UN_FOLLOW = "0";
    public static final int FOLLOW_INTEGER = 1;
    public static final int UN_FOLLOW_INTEGER = 0;
    public static final String ROOT_GET_NEWS = "http://topica.ai:6968/api/v1/news/"; //for all search

    public static final String URL_GET_ROOT_LOG_IN = "http://topica.ai:6968/api/v1/user/"; //Dùng cho follow

    public static final String URL_GET_VIDEO = "http://topica.ai:6968/api/v1/news/channel/";

    public static final String IP_ADDRESS_LOGGING = "http://110.35.75.45:2611/logging/";

//    public static final String URL_SEARCH_POST = "http://topica.ai:6996/";
}
