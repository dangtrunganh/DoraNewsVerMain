...
public class VideoFragment2 extends BaseFragmentNeedUpdateUI implements UpdateListNotification, View.OnClickListener {
...
//    private boolean loadAllVideos = false;
...
//        recyclerViewVideos.setAdapter(videoAdapter);
        loadData(LoadPageConst.RELOAD_INIT_CURRENT_PAGE);
        swipeContainer.setOnRefreshListener(() -> loadData(LoadPageConst.RELOAD_INIT_CURRENT_PAGE));
        setUpLoadMore();
    }
...
//                    return;
//                }
                loadData(LoadPageConst.LOAD_MORE_PAGE);
...
//                }
    ...
    private void loadData(int typeLoadData) {
        if (GeneralTool.isNetworkAvailable(Objects.requireNonNull(mContext))) {
            ...
            Retrofit retrofit = new Retrofit.Builder()
                    .baseUrl(RootAPIUrlConst.URL_GET_VIDEO)
                    .addConverterFactory(GsonConverterFactory.create())
                    .build();

            final ServerAPI apiService = retrofit.create(ServerAPI.class);
            Call<VideoResult> call = apiService.getListVideos(String.valueOf(typeLoadData), deviceId, uId);
...
//                        Toast.makeText(mContext, "Error: Call API successfully, but data is null!", Toast.LENGTH_SHORT).show();
                    ...
                    ArrayList<VideoItem> arrayListVideosTemp = videoResult.getData();
//                    if (videoAdapter.getArrayVideos().size() == 1) {
//                        //Hiển thị màn hình chưa có sự kiện nào được theo dõi
//                        //Luôn luôn tồn tại một phần tử null làm footer
//                        imageWifiOff.setImageResource(R.drawable.ic_no_video_youtube);
...
//                    } else {
                    ...
                    recyclerViewVideos.setVisibility(View.VISIBLE);
                    //update
                    if (typeLoadData == LoadPageConst.RELOAD_INIT_CURRENT_PAGE) {
                        //load lần đầu
